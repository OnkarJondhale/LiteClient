...
function ApiMock() {
    const {
        mocks, activeMockId, setActiveMockId, addMock,
        removeMock, updateActiveMock, getActiveMock
    } = useMockStore();
    ...
    const [activeTab, setActiveTab] = useState(() => {
        return sessionStorage.getItem('lite-client-active-tab') || "Params";
    });

    useEffect(() => {
        sessionStorage.setItem('lite-client-active-tab', activeTab);
    }, [activeTab]);
        ...
        <div className="flex h-full text-[14px] text-[#18181b] dark:text-[#fafafa] bg-white dark:bg-[#09090b] overflow-hidden">
            ...
            <div className={`${isSidebar ? 'w-64' : 'w-0'} transition-all border-r border-[#e4e4e7] dark:border-[#27272a] flex flex-col shrink-0 overflow-hidden`}>
                <div className="p-3.5 border-b border-[#e4e4e7] dark:border-[#27272a] flex justify-between items-center whitespace-nowrap">
                    ...
                    <button onClick={addMock} className="text-blue-600 font-bold text-lg hover:scale-110 transition-transform">+</button>
                </div>
                    ...
                    <div className='h-full flex items-center space-x-6 px-4'>
                        <div className="relative flex items-center group">
                            <label className="inline-flex items-center cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={activeMock.isActive}
                                    onChange={(e) => updateActiveMock({ isActive: e.target.checked })}
                                    className="sr-only peer"
                                />
                                <div className="relative w-9 h-5 bg-zinc-200 dark:bg-zinc-700 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:start-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600 dark:peer-checked:bg-blue-500"></div>
                                <span className="select-none ms-3 text-xs font-medium text-[#71717a] dark:text-[#a1a1aa]">Activate URL</span>
                            </label>
                            <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 hidden group-hover:block z-50 px-3 py-1.5 text-[11px] font-medium text-white bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900 rounded shadow-md whitespace-nowrap pointer-events-none">
                                {activeMock.isActive ? 'Mock is LIVE' : 'Mock is DISABLED'}
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 border-8 border-transparent border-b-zinc-900 dark:border-b-zinc-100"></div>
                            </div>
                        </div>
